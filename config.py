...
class Configurations:
    ...
    def __init__(self):
        ...
        if not premium_economy_role_id_str:
            raise ValueError("PREMIUM_ECONOMY_ROLE_ID environment variable is required!")
    ...
    def save(self, file_path="/data/config/bot_settings.json"):
        ...
        data = {
            ...
            "lh_mh_attributes": self.lh_mh_attributes,
        }
    ...
    def load(self, file_path="/data/config/bot_settings.json"):
        if os.path.exists(file_path):
            with open(file_path, "r") as file:
                ...
                self.lh_mh_attributes = data.get("lh_mh_attributes", {})
            if not self.lh_mh_attributes: 
                self.lh_mh_attributes = {
                    "departure_airport": "N/A",
                    "arrival_airport": "N/A",
                    "airline": "N/A",
...
class FlightHours:
    def __init__(self):
        ...
        self.event_history = OrderedDict()  # Key: Event Name (str) | Value: ID of Members Joined (Set of str)
    ...
    def log_start_time(self, member_id, member=None):
        ...
        if not member:
            ...
            try:
                guild_member = self.guild.get_member(int(member_id_str)) if self.guild else None
    ...
    def log_end_time(self, member_id, member=None):
        ...
        with self._lock:
            if member_id_str in self.start_time:
                ...
                if minutes_flown < 0: minutes_flown = 0
        ...
        if os.path.exists(file_path):
            try:
                with open(file_path, "r") as file:
                    ...
                    self.voice_channels = [config.guild.get_channel(vc_id) for vc_id in data.get("voice_channels", []) if config.guild.get_channel(vc_id) is not None]
                    ...
                    self.event_history = OrderedDict((k, set(v)) for k, v in event_history_data.items())
                ...
                if os.path.exists(backup_path):
                    try:
                        with open(backup_path, "r") as file:
                            ...
                            event_history_data = data.get("event_history", {})
                            self.event_history = OrderedDict((k, set(v)) for k, v in event_history_data.items())
                            print(f"Restored flight hours from backup due to corruption: {e}")
