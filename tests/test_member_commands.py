...
class TestMemberCommandUtilities:
    ...
    def test_event_history_logic(self):
        ...
        user_id = "user1"
        user_events = [
            event for event, attendees in event_history.items() if user_id in attendees
        ]
        ...
        event_attendance_counts = {
            event: len(attendees) for event, attendees in event_history.items()
        }
