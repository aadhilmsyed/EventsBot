...
class TestModCommandUtilities:
    ...
    def test_role_permission_checking(self):
        ...
        user_roles = [
            989232534313369630
        ]  # First class role (insufficient for mod commands)
        has_first_officer = first_officer_role_id in user_roles
        has_captain = captain_role_id in user_roles
    ...
    def test_channel_management_logic(self):
        ...
        restricted_channels = [
            int(os.getenv("LOG_CHANNEL_ID")),
            int(os.getenv("MESSAGE_LOGS_CHANNEL_ID")),
        ]  # LOG_CHANNEL_ID, MESSAGE_LOGS_CHANNEL_ID
        new_channel_id = int(
            os.getenv("MEMBER_LOGS_CHANNEL_ID")
        ...
        )  # LOG_CHANNEL_ID (already in list)
    ...
    def test_data_export_logic(self):
        ...
        flight_hours = {"user1": 120, "user2": 180, "user3": 90}
    ...
    def test_permission_hierarchy_logic(self):
        ...
        def has_permission(user_role, required_role):
            return role_hierarchy.get(user_role, 999) <= role_hierarchy.get(
                required_role, 999
            )
