...
class TestMonthlyRolesUtilities:
    ...
    def test_calculate_earned_role_function(self):
        ...
        assert calculate_earned_role(30) == int(os.getenv('MODERATOR_ROLE_ID'))  # 0.5 hours + 1 grace = 1 hour (Moderator)
        assert calculate_earned_role(60) == int(os.getenv('CAPTAIN_ROLE_ID'))  # 1 hour + 1 grace = 2 hours (Captain)
        assert calculate_earned_role(90) == int(os.getenv('CAPTAIN_ROLE_ID'))  # 1.5 hours + 1 grace = 2.5 hours (Captain)
        assert calculate_earned_role(120) == int(os.getenv('FIRST_OFFICER_ROLE_ID'))  # 2 hours + 1 grace = 3 hours (First Officer)
        assert calculate_earned_role(180) == int(os.getenv('FIRST_CLASS_ROLE_ID'))  # 3 hours + 1 grace = 4 hours (First Class)
        assert calculate_earned_role(300) == int(os.getenv('PREMIUM_ECONOMY_ROLE_ID'))  # 5 hours + 1 grace = 6 hours (Premium Economy)
    ...
    def test_role_comparison_logic(self):
        ...
        def find_best_role(hours):
            ...
            for role_name, role_data in roles.items():
                if hours >= role_data["threshold"] and role_data["threshold"] > best_threshold:
        ...
        assert find_best_role(4) == 123456789012345682  # Still First Officer (highest available)
    ...
    def test_flight_hours_data_processing(self):
        ...
        active_users = {user: hours for user, hours in flight_hours.items() if hours > 0}
        ...
        role_assignments = {user: assign_role(hours) for user, hours in active_users.items()}
        expected_roles = {
            "user1": "captain",
            "user2": "first_officer",
            "user3": "moderator",
    ...
    def test_data_cleanup_logic(self):
        ...
        start_times = {"user1": "2023-01-01", "user2": "2023-01-02", "user3": "2023-01-03"}
