...
class TestMainIntegration:
    ...
    async def test_bot_startup_and_ping_command(self):
        ...
        try:
            ...
            ), f"Expected 'ping' command, got '{ping_command.name}'"
            ...
            if ctx.command:
                ...
                assert (
                    ping_response_content is not None
                ), "Ping command did not generate response"
                assert (
                    "Pong!" in ping_response_content
                ), f"Expected 'Pong!' in response, got: {ping_response_content}"
                assert (
                    "Latency:" in ping_response_content
                ), f"Expected 'Latency:' in response, got: {ping_response_content}"
                ...
                assert (
                    len(sent_messages) > 0
                ), "Command did not attempt to send a message"
                assert (
                    "Pong!" in sent_messages[0]
                ), f"Expected 'Pong!' in sent message, got: {sent_messages[0]}"
    ...
    async def test_bot_latency_check(self):
        ...
        assert math.isnan(
            test_bot.latency
        ), f"Expected NaN latency, got {test_bot.latency}"
